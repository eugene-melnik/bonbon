...
void* connect_thread( void* button )
{
    ...
    if( !strcmp( global.hostname, EMPTY_STRING ) ||
        !strcmp( global.username, EMPTY_STRING ) )
    {
        gdk_threads_enter();
    ...
    {
        /* Connecting */
        if( ( global.main_pipe = open_connection( global.hostname, global.username, global.password) )
                              != NULL)
        {
